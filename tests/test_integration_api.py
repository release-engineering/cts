...
def test_composes_pagination(write_http_client):
    ...
    status, data = write_http_client.get("/api/1/composes/?page=1&per_page=2")
    assert status == 200
    assert isinstance(data, dict)
    assert "items" in data
    assert "meta" in data
    ...
    status, data = write_http_client.get("/api/1/composes/?page=2&per_page=2")
    assert status == 200
    assert "items" in data
    assert (
        len(data["items"]) >= 1
...
def test_workflow_tag_creation(write_http_client):
    ...
    # Step 2: Add a tagger
    data = add_tagger(write_http_client, tag_id, "test-user")
    ...
    data = add_tagger(write_http_client, tag_id, "another-user")
    ...
    data = remove_tagger(write_http_client, tag_id, "test-user")
    ...
    status, final_data = write_http_client.get(f"/api/1/tags/{tag_id}")
    assert status == 200
...
def test_auth_builder_can_post_compose(auth_http_client_builder):
    ...
    assert (
        status == 200
    ), f"Expected 200 for authenticated builder POST, got {status}. Response: {data}"
    assert isinstance(data, dict)
    assert "payload" in data
    assert "compose" in data["payload"]
    compose_id = data["payload"]["compose"]["id"]
